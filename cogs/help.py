...
class help(commands.Cog):
    ...
    async def help(self,ctx,category=None):
        ...
        if category == None:
        ...
        elif category == "mod" or category == "Mod" or category == "Moderation" or category == "moderation":
            cog = self.client.get_cog('moderation')
            commands = cog.get_commands()
            embed = discord.Embed(title="Athena Moderation Commands",colour=discord.Colour.from_rgb(255,192,203),description='Commands with () are optional and [] is required.',timestamp=ctx.message.created_at)
            embed.set_footer(text=f"Requested by {ctx.author}",icon_url=ctx.author.avatar_url)
            embed.set_thumbnail(url="https://cdn.discordapp.com/attachments/863200845688733736/863930582320939029/original.jpg")
            ...
            if ctx.author.id == 798325945463078952 or ctx.author.id == 746051373753171978:
